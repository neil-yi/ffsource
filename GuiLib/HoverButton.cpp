...
//---------------
	int nButtonW = pHoverBitmaps[0]/3;
	int nButtonH = pHoverBitmaps[1];

	CRect sr(0,0, nButtonW , nButtonH );

	if( lpDrawItemStruct->itemState & ODS_SELECTED )
	{
		sr.OffsetRect( nButtonW * 2 , 0 );
	}
	else
	{
		if( m_bHover )
		{
			sr.OffsetRect( nButtonW , 0 );
		}
	}

	SetRawPixels( pDC , pHoverBitmaps , &CRect(0,0,nButtonW,nButtonH) , &sr );

	pDC->SetBkMode(TRANSPARENT);
	pDC->DrawText( "..." , &cr , DT_SINGLELINE | DT_CENTER | DT_VCENTER );
...
afx_msg LRESULT CHoverButton::OnMouseHover(WPARAM wparam, LPARAM lparam) 
{
	// TODO: Add your message handler code here and/or call default
	m_bHover=TRUE;
	Invalidate();
