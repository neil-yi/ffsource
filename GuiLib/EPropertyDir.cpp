...
bool EPropertyDir::OnCommand(EPropWnd* pParent , WPARAM wParam , LPARAM lParam )
{
	...
	bwinfo.pszDisplayName=(TCHAR*) buffer;//will receive folder name (as displayed) 
	bwinfo.lpszTitle=_T("Select the folder");//dialog title 
	...
	{ 
		SHGetPathFromIDList(itemids,(TCHAR*) folder_path);//get the folder path from SHBrowseForFolder result 
