...
class GUILIBDLLEXPORT EPropertyDouble : public ENumericProperty
{
...
public:
    ...
    virtual bool PropertyChanging( const void* pProperty , void* pNewValue )
	{
		...
		for( UINT i=0 ; i<m_vTargets.size() ; i++ )
		{
	...
	virtual CString GetTextValue()
	{
		...
		if( m_vTargets.size()>=0 )
		{
			...
			for( UINT i=1 ; i<m_vTargets.size() ; i++ )
			{
