...
void CGuiMenuButton::OnLButtonDown(UINT nFlags, CPoint point)
{
	...
	//ATLTRACE( _T("PROBLEM IS AROUND HERE : %d\n"), nCtrlID);
	GetParent()->SendMessage(WM_LBUTTONDOWN, 100+GetDlgCtrlID()-1, MAKEWPARAM(point.x,point.y));
}
...
CMenuButton::CMenuButton(HMENU hMenu, int nIndex,CWnd* pWnd)
{
	...
	// WARNING : When calling Create we cannot have a zero index - so I add 1
	// and I substract it when calling GetCtrlID
	DWORD dwFlags = WS_VISIBLE | WS_CHILD | BS_PUSHBUTTON | BS_OWNERDRAW;
	if (m_bt.Create(_T(""), dwFlags, CRect(0,0,0,0), pWnd, nIndex+1) == TRUE)
	{
...
CMenuControl::CMenuControl(CWnd* pMenuBar)
{
	...
	{
		//TRACE0(_T("CMenuControl::CMenuControl(): error\n"));
	}
