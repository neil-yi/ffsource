...
CString EPropList::GetChildHostName()
{
	return m_sChildHostName;
}
...
{
	EPropertyInt* p = new EPropertyInt( pHost , sName , pProperty , sUnit , bEnabled , 0 , 0 );
	AddProperty(p);
	return p;
}

EPropertyFloat* EPropList::AddPropFloat( IPropertyHost* pHost , const CString& sName , float* pFloat , const CString& sUnit , bool bEnabled )
{
	EPropertyFloat* p = new EPropertyFloat(pHost,sName,pFloat,sUnit,0,0,bEnabled);
	AddProperty(p);
	return p;
}

EPropertyDouble* EPropList::AddPropDouble( IPropertyHost* pHost , const CString& sName , double* pDouble , const CString& sUnit , bool bEnabled )
{
	EPropertyDouble* p = new EPropertyDouble(pHost,sName,pDouble,sUnit,0,0,bEnabled);
	AddProperty(p);
	return p;
}
...
EPropertySeparator* EPropList::AddPropSeparator( IPropertyHost* pHost , const CString& sName )
{
	EPropertySeparator* p = new EPropertySeparator(pHost,sName);
	AddProperty(p);
	return p;
}
...
void EPropList::AddPropHost( IPropertyHost* pHost , const CString& sName , IPropertyHost* pChildHost )
{
	if( pHost!=NULL && pChildHost!=NULL )
	{
		m_sChildHostName = sName;
		m_vParentStack.push( pHost );
		pChildHost->GetProperties(*this);
		m_sChildHostName = "";
		m_vParentStack.pop();
...
EPropertyDir* EPropList::AddPropDir(IPropertyHost* pHost , const CString& sName , CString* pDir )
{
	EPropertyDir* p = new EPropertyDir(pHost,sName,pDir);
	AddProperty(p);
	return p;
}
...
EPropertySystemTime* EPropList::AddPropSystemTime( IPropertyHost* pHost , const CString& sName , SYSTEMTIME* pSystemTime )
{
	EPropertySystemTime* p = new EPropertySystemTime(pHost,sName,pSystemTime);
	AddProperty(p);
	return p;
}
