...
IMPLEMENT_DYNCREATE(EPropListWnd, EPropWnd)

BEGIN_MESSAGE_MAP(EPropListWnd, EPropWnd)
	//{{AFX_MSG_MAP(EPropListWnd)
	ON_WM_ERASEBKGND()
	ON_WM_MOUSEMOVE()
	ON_WM_LBUTTONDOWN()
	ON_WM_LBUTTONUP()
...
void EPropListWnd::OnSetFocus( CWnd* pOldWnd )
{
	TR( _T("EPropListWnd::OnSetFocus()") , _T("not doing anything") );
}

void EPropListWnd::OnKillFocus( CWnd* pNewWnd )
{
	TR( _T("EPropListWnd::OnKillFocus()") , (pNewWnd==NULL) ? _T("NULL") : _T("?") );
}

void EPropListWnd::OnTimer( UINT nTimer )
{
	Invalidate();
}

//afx_msg void EPropListWnd::QQQ()
//{
//	TRACE("RR");
//}

EPropListWnd::EPropListWnd()
{
...
//
void EPropListWnd::_UpdateDrawnPropertiesList( CDC* pDC )
{
	//
	...
	m_nOptimalValColumnWidth += m_nEdgeWidth * 2;
...
//DWORD nLastPaintTime = 0;
//int nPropListPaintCount = 0;
void EPropListWnd::OnPaint()
{
//	DWORD nNow = GetTickCount();
//	TRACE("onpaint %d delta=%d\n" , nPropListPaintCount++ , nNow-nLastPaintTime );
//	nLastPaintTime = nNow;
...
//	}
	...
	else
	{
		...
		for( int i=0 ; i<m_nNumDrawnProperties ; i++ )
		{
			...
			if( pProp != GetActiveProperty() )	//never skip active property
			{
...
//			}
			...
			ASSERT( pProp->m_Rect.left  == 0 );
			ASSERT( pProp->m_Rect.right == 0 );
		}
...
/*
void EPropListWnd::OnKeyDown(UINT nChar, UINT nRepCnt, UINT nFlags) 
{
	TRACE("view keydown\n");
	if( nChar == 27 )	//escape
	{
...
void EPropListWnd::OnLButtonUp(UINT nFlags, CPoint cp) 
{
	m_nMouseDownMousePositionSP  = -1;
	m_nMouseDownKeyColumnWidth   = -1;
	ReleaseCapture();
...
//void EPropListWnd::OnLButtonDblClk( UINT nFlags , CPoint point)
//{
//	TRACE("Double Click\n");
...
void EPropListWnd::OnLButtonDown(UINT nFlags, CPoint cp) 
{
	TR( _T("EPropListWnd::OnLButtonDown()"), _T("") );

	CPoint sp = _CP2SP(cp);

	//
	// set focus to this control (killing focus of active edit)
	// eg when clicking splitter, or clicking active property
	//

	if( GetActiveProperty() == NULL )
	{
		//only set focus if none is currently selected. if a property is selected
		//if a property looses focus (because focus is now changed to this) it doesnt
		//have a change to veto the focus change. it's a bit of a hack, but fairly
		//ok for now. Veto feature turns out to be quite complicated..
		SetFocus();
	}
	...
	else										//begin edit another property?
	{
		...
		//
		// find the property that was clicked, the new property
		//

		bool bPropertyClicked = true;
		...
		for( int i=0 ; i<m_PropList.Size() ; i++ )
		{
			...
			if( pProp->m_Rect.Height() > 0 )
			{
				if( (pProp->m_Rect.top<=sp.y) && (sp.y<pProp->m_Rect.bottom) )
				{
					...
					if( pProp->m_bHasChildren )	//if the clicked property has children
					{
						if( pProp->m_NodeOpenRect.PtInRect(sp) )	//check if the open-node icon was hit?
						{
							bPropertyClicked = false;
...
//			{
//				if( pNewProperty->IsSelectable()==false )	//can be selected?
//				{
//					pNewProperty = NULL;
//				}
//			}

			if( SetActiveProperty(pNewProperty,true,true) )
			{
				if( GetActiveProperty() != NULL )
				{
					if( GetActiveProperty()->OnLButtonDown(this,sp) )	//send a left button down to te new property (check property uses this call to toggle)
					{
...
void EPropListWnd::OnMouseMove(UINT nFlags, CPoint cp) 
{
	CPoint sp = _CP2SP(cp);
...
BOOL EPropListWnd::OnEraseBkgnd(CDC* pDC) 
{
	return FALSE;
}
...
CSize EPropListWnd::GetScrollPageSize()
{
	return m_ScrollPageSize;
}

bool EPropListWnd::SetActiveTab(int i)
{
	bool bNewTab = false;
	...
	{
		if( SetActiveProperty(NULL,false,false) )
		{
			bNewTab = true;
...
EPropList* EPropListWnd::GetPropertyList()
{
	return &m_PropList;
}

CString EPropListWnd::GetCurrentComment( CString& sObjectName )
{
	CString s;
	...
	if( m_pPropertyHost != NULL )
	{
		if( GetActiveProperty()!=NULL )
		{
			s = GetActiveProperty()->GetComment();
			sObjectName = GetActiveProperty()->GetName();
		}
		else
		{
...
void EPropListWnd::SetPropertyHost( IPropertyHost* pHost )
{
	//
...
//	ApplyActiveProperty();
	SetActiveProperty(NULL,true,false);
	ASSERT( GetActiveProperty() == NULL );
...
void EPropListWnd::AutoSizeKeyColumn()
{
	//
	...
	if ( nActiveTab>=0 )
	{
		...
		int nMargins = (m_nEdgeWidth + 4) * 2;

		EPropList::ETabGroup* pTab = m_PropList.GetTab(nActiveTab);
		int n = pTab->Size();

		for( int i=0 ; i<n ; i++ )
		{
			EProperty* pProp = pTab->Get(i);

			CString s = pProp->GetName();

			int nIndent = pProp->m_nIndent * 13;
			int	nNode   = pProp->m_bHasChildren ? (pNodeClosed[0]+4) : 0;
			int nText   = dc.GetTextExtent(s).cx;

			int nTotal = nIndent + nText + nNode + nMargins;

			if( nTotal > m_nKeyColumnWidth )
			{
				m_nKeyColumnWidth = nTotal;
			}
		}
	}
}

void EPropListWnd::RefreshHostList()
{
	SetPropertyHost(m_pPropertyHost);
}

void EPropListWnd::SetPropertyChangeListener( IPropertyChangeListener* pListener )
{
...
void EPropListWnd::StepSelection( int nStep )
{
//------------
//	int nNumProperties = m_PropList.Size();
//
...
//-------------
	...
	for( int i=0 ; i<m_nNumDrawnProperties ; i++ )
	{
		if( m_pDrawnProperties[i] == GetActiveProperty() )
		{
			i += nStep;
			...
			SetActiveProperty( pNewActive , true,true);
//			EnsureVisibility( pNewActive );
...
bool EPropListWnd::ValidateItems()
{
	bool bValid = true;

	if( GetActiveProperty() != NULL )
	{
		bValid = GetActiveProperty()->CanApply();
	}
...
BOOL EPropListWnd::PreTranslateMessage( MSG* pMsg )
{
//	if( pMsg->message == WM_KEYDOWN )
//	{
//		TRACE("EPropListWnd::PreTranslateMessage(WM_KEYDOWN)\n");
//	}
	...
	if( pMsg->message == WM_KEYDOWN )
	{
//		TR("EPropListWnd::PreTranslateMessage()" , "WM_KEYDOWN" );
		...
		{
			SetActiveProperty(NULL,true,true);
		}
		else if(pMsg->wParam == VK_ESCAPE)
		{
//			m_bApplyOnKillFocus = false;
			SetActiveProperty(NULL,false,false);
		}
		...
		else if(pMsg->wParam == VK_RIGHT )
		{
			...
			if( GetActiveProperty() != NULL )
			{
				if( GetActiveProperty()->m_bHasChildren )
				{
					if( GetFocus() == this )		//not the edit
					{
						if( GetActiveProperty()->m_bNodeOpen == false )
						{
							OpenNode( GetActiveProperty() );
							bHandled = TRUE;
						}
					}
				}
		...
		else if(pMsg->wParam == VK_LEFT )
		{
			...
			if( GetActiveProperty() != NULL )
			{
				if( GetActiveProperty()->m_bHasChildren )
				{
					if( GetFocus() == this )		//not the edit
					{
						if( GetActiveProperty()->m_bNodeOpen == true )
						{
							CloseNode( GetActiveProperty() );
							bHandled = TRUE;
						}
					}
				}
		...
		else if(pMsg->wParam == VK_SPACE )
		{
			...
			if( GetActiveProperty() != NULL )
			{
				if( GetFocus() == this )
				{
					if( GetActiveProperty()->OnSpaceKey() )
					{
						NotifyPropertyChanged();
						Invalidate();
						bHandled = TRUE;
					...
					if( GetActiveProperty()->m_bHasChildren )
					{
						if( GetActiveProperty()->m_bNodeOpen )
						{
							CloseNode( GetActiveProperty() );
						}
						else
...
void EPropListWnd::CloseNode( EProperty* pProp )
{
	if( pProp == NULL )
	{
		return;
	...
	//must close the active property (if the active property is under this closing node, it would keep hanging if it wasnt closed)
	CloseActiveProperty();
//	EProperty* pActiveProperty = GetActiveProperty();
//	if( pActiveProperty != NULL )
//	{
//		pActiveProperty->OnPropClose();
//	}
...
void EPropListWnd::OpenNode( EProperty* pProp )
{
	if( pProp == NULL )
	{
		return;
