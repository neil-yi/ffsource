...
public:
	CharacterRange getSelection() {
		...
		crange.cpMin = rCtrl.GetSelectionStart();
		crange.cpMax = rCtrl.GetSelectionEnd();
		return crange;
	};

	int getCurrentDocLen() {
		CScintillaCtrl& rCtrl = GetCtrl();
		return rCtrl.GetLength();
	};

	void clearIndicator(int indicatorNumber) {
		CScintillaCtrl& rCtrl = GetCtrl();
		int docStart = 0;
		int docEnd = rCtrl.GetLength();
		rCtrl.SetIndicatorCurrent( indicatorNumber);
		rCtrl.IndicatorClearRange( docStart, docEnd-docStart);
	};
