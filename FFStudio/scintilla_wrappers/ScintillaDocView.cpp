...
BEGIN_MESSAGE_MAP(CScintillaView, CView)
  ...
  //ON_COMMAND(ID_EDIT_ADDCOMMENT, &CScintillaView::OnAddComment)
  ON_WM_SETFOCUS()
  ON_WM_SIZE()
  ON_WM_ERASEBKGND()
  ON_WM_CREATE()
...
void CScintillaView::OnAddComment()
{
	CString str = "add comment";
	AfxMessageBox(str);
}
...
std::pair<int, int> CScintillaView::getWordRange()
{
	CScintillaCtrl& rCtrl = GetCtrl();
    int caretPos = rCtrl.GetCurrentPos();
	int startPos = rCtrl.WordStartPosition(caretPos, true);
	int endPos = rCtrl.WordEndPosition(caretPos, true);
	return std::pair<int, int>(startPos, endPos);
}

bool CScintillaView::expandWordSelection()
{
	CScintillaCtrl& rCtrl = GetCtrl();
	std::pair<int, int> wordRange = 	getWordRange();
    if (wordRange.first != wordRange.second) {
		rCtrl.SetSelectionStart(wordRange.first);
		rCtrl.SetSelectionEnd(wordRange.second);
		return true;
	}
	return false;
}

void CScintillaView::getText(char *dest, int start, int end)
{
	CScintillaCtrl& rCtrl = GetCtrl();
	TextRange tr;
	tr.chrg.cpMin = start;
...
char * CScintillaView::getWordFromRange(char * txt, int size, int pos1, int pos2)
{
    if (!size)
		return NULL;
    if (pos1 > pos2)
...
char * CScintillaView::getSelectedText(char * txt, int size, bool expand)
{
	if (!size)
		return NULL;
